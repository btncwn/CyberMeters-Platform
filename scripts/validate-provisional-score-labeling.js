...
for (const [label, score, quality] of fixtures) {
  ...
  eq(`${label}: methodology identity is explicit`, result.methodology_version, '2026-07-16.1')
}
...
ok('Cyber Metrics Score algorithm and methodology identity remain unchanged',
  /score \+= f\.score_impact;/.test(scoring) &&
    /CYBER_METRICS_SCORE_METHODOLOGY_VERSION\s*=\s*"2026-07-16\.1"/.test(scoring))
