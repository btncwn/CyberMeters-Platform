...
const MUTANTS = [
  ...
  { id:"U1-M12", file:"contract", anchor:"ORDER BY first_seen ASC, created_at ASC, id ASC", replacement:"ORDER BY first_seen ASC", fail:["U1-C02","U1-C04","U1-C05"], controls:["U1-C01"] },
  ...
  { id:"U1-M20", file:"contract", anchor:"export const IDENTITY_CANONICAL_EXPOSURE_QUERY = `${CANONICAL_IDENTITY_CTE}\nSELECT hostname, identity_type, provider, internet_exposed, risk_score\n  FROM canonical_identity_assets\n ORDER BY risk_score DESC, internet_exposed DESC, id ASC LIMIT 100`;", replacement:"export const IDENTITY_CANONICAL_EXPOSURE_QUERY = `SELECT hostname, identity_type, provider, internet_exposed, risk_score FROM identity_assets WHERE workspace_id = ? AND status = 'active' ORDER BY risk_score DESC, internet_exposed DESC, id ASC LIMIT 100`;", fail:["U1-CONS06"], controls:["U1-POS-UNIQUE"] },
  { id:"U1-M21", file:"contract", anchor:"export const IDENTITY_CANONICAL_SHADOW_QUERY = `${CANONICAL_IDENTITY_CTE}\nSELECT id, hostname, provider, risk_score, first_seen, last_seen\n  FROM canonical_identity_assets\n WHERE provider IS NOT NULL AND provider != ''`;", replacement:"export const IDENTITY_CANONICAL_SHADOW_QUERY = `SELECT id, hostname, provider, risk_score, first_seen, last_seen FROM identity_assets WHERE workspace_id = ? AND status = 'active' AND provider IS NOT NULL AND provider != ''`;", fail:["U1-CONS07"], controls:["U1-POS-UNIQUE"] },
