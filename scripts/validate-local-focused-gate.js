...
const EXPECTED_POLICY_FINGERPRINT = "94ec75c690f143a5aae2a6d588a548bf600715c14d187db67a744d14fb390905";
...
ok("pack order is exact", JSON.stringify(PACK_ORDER) === JSON.stringify([
  "runtime_evidence", "report_cx", "ci_policy", "recovery_assurance", "docs_only",
]));
...
const recovery = classify(["scripts/ops/restore-production-backup-to-staging.sh"]);
ok("restore script selects recovery assurance", recovery.decision === "FOCUSED" && recovery.selected_packs[0] === "recovery_assurance");
ok("restore script adds exactly F004-M010 sentinel",
  recovery.commands.at(-1).id === "recovery-sentinel-F004-M010" &&
  recovery.commands.filter((item) => item.id.startsWith("recovery-sentinel")).length === 1);
const threePack = classify([
  "scripts/validate-scan-deadline.js",
  "scripts/validate-report-first-cx.js",
  "scripts/validate-f004-recovery-mutation-sharding.js",
]);
ok("three-pack rescue lane remains focused", threePack.decision === "FOCUSED" &&
  JSON.stringify(threePack.selected_packs) === JSON.stringify(["runtime_evidence", "report_cx", "recovery_assurance"]));
ok("three-pack command order follows fixed pack order",
  ...
  threePack.commands.findIndex((item) => item.id === "report-pdf-syntax") <
  threePack.commands.findIndex((item) => item.id === "recovery-instrumentation-syntax"));
...
ok("four selected packs require full assurance", classify([
  "scripts/validate-scan-deadline.js",
  "scripts/validate-report-first-cx.js",
...
// A failed F004 sentinel remains FAILED; the local smoke never waives 53/53 PR proof.
{
  const repo = makeRepo({ "scripts/ops/backup-production-data.sh": "#!/bin/sh\nexit 0\n" });
  const lockRoot = fs.mkdtempSync(path.join(os.tmpdir(), "local-focused-locks-"));
  try {
    write(repo, "scripts/ops/backup-production-data.sh", "#!/bin/sh\nexit 1\n");
    const run = runLocalFocusedGate({
      argv: ["--worktree"], repoRoot: repo,
      hooks: successfulHooks(lockRoot, {
        executeCommand: (item) => ({
          status: item.id === "recovery-sentinel-F004-M001" ? 1 : 0,
          signal: null, error: null, stdout: "", stderr: "",
        }),
      }),
    });
    ok("surviving/failed recovery sentinel makes the focused gate fail",
      run.exitCode === 1 && /recovery-sentinel-F004-M001/.test(run.result.reason));
  } finally {
    removeRepo(repo);
    fs.rmSync(lockRoot, { recursive: true, force: true });
  }
