...
const mutants = [
  ...
  { id: "TRIAGE-M7-D3-methodology-not-bumped", file: "workers/scan-api/src/engines/scoring.js",
    from: 'export const CYBER_METRICS_SCORE_METHODOLOGY_VERSION = "2026-08-26.1";',
    to:   'export const CYBER_METRICS_SCORE_METHODOLOGY_VERSION = "2026-08-23.1";',
    mustContain: "FAIL D3: methodology stamp remains newer than the pre-D3 contract" },
  ...
  { id: "TRIAGE-M11-D3-current-posture-revived", file: "workers/scan-api/src/engines/current-posture.js",
    // Anchor re-derived after the P1-2 websiteWithheld ternary replaced the
    // bare assignment; the mutant still revives the persisted score whenever
    // Phase-5 intelligence evidence reads complete.
    from: "    const value = websiteWithheld",
    to:   "    const value = phase5.evidence.complete && !websiteWithheld\n      ? { ...phase5.assessment, raw_score: row.score, display_score: row.score, display_rating: row.rating, authoritative: true, provisional: false }\n      : websiteWithheld",
  ...
  { id: "TRIAGE-M15-D3-persisted-kev-deducted-twice", file: "workers/scan-api/src/engines/phase5-evidence.js",
    ...
    mustContain: "FAIL D3 persistence boundary: historical projection preserves persisted 85/good" },
  { id: "TRIAGE-M17-D3-score-absence-reconflated", file: "workers/scan-api/src/engines/phase5-evidence.js",
    from: "  if (skippedScored.length > 0 || (suppressOnEvidenceGaps && !evidence.complete)) {",
    to:   "  if (skippedScored.length > 0 || (suppressOnEvidenceGaps && !evidence.complete) || !Number.isFinite(score)) {",
    mustContain: "FAIL D3 boundary: complete row with empty score cell keeps scan_quality complete" },
  { id: "TRIAGE-M18-P1-hosted-budget-gated-again", file: "workers/scan-api/src/engines/dmarcbis-resolver.js",
    from: "    if (isCybermetersHostedDestination(uri.destination_host, hostedRuaDomain)) {\n      const hosted = await resolveOneExternalHost({",
    to:   "    if (false && isCybermetersHostedDestination(uri.destination_host, hostedRuaDomain)) {\n      const hosted = await resolveOneExternalHost({",
    mustContain: "FAIL D4-P1: hosted destination survives a refused reservation" },
  { id: "TRIAGE-M19-P1-invalidation-not-atomic", file: "workers/scan-api/src/engines/phase5-evidence.js",
    from: "      summary: null,\n      assessment: resolveAssessmentPresentation({",
    to:   "      summary: overall.summary ?? null,\n      assessment: overall.assessment ?? resolveAssessmentPresentation({",
    mustContain: "FAIL P1-2: stale assessment cannot stay authoritative" },
  { id: "TRIAGE-M20-P1-posture-ignores-invalidation", file: "workers/scan-api/src/engines/current-posture.js",
    from: "    const websiteWithheld = websiteRedirectConclusionWithheld(phase5Modules, reportFindings);",
    to:   "    const websiteWithheld = false;",
    mustContain: "FAIL P1-2 current posture: a withdrawn redirect conclusion cannot be authoritative" },
