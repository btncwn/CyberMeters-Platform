...
const REPORT_CX_COMMANDS = Object.freeze([
  ...
  command("report-first-cx", ["node", "scripts/validate-report-first-cx.js"]),
  command(
...
const RECOVERY_COMMANDS = Object.freeze([
  command("recovery-instrumentation-syntax", ["node", "--check", "scripts/validate-f004-recovery-instrumentation.js"]),
  command("recovery-mutations-syntax", ["node", "--check", "scripts/validate-f004-recovery-instrumentation-mutations.js"]),
  command("recovery-sharding-syntax", ["node", "--check", "scripts/validate-f004-recovery-mutation-sharding.js"]),
  command("recovery-instrumentation", ["node", "scripts/validate-f004-recovery-instrumentation.js"]),
  command("recovery-mutation-list", ["node", "scripts/validate-f004-recovery-instrumentation-mutations.js", "--list"]),
  command("recovery-mutation-sharding", ["node", "scripts/validate-f004-recovery-mutation-sharding.js"]),
]);
...
export const PACKS = Object.freeze({
  ...
  recovery_assurance: Object.freeze({ id: "recovery_assurance", commands: RECOVERY_COMMANDS }),
...
const PATH_PACKS = new Map([
  ...
  ["scripts/validate-report-first-cx.js", ["report_cx"]],
  ...
  ["scripts/ops/backup-production-data.sh", ["recovery_assurance"]],
  ["scripts/ops/restore-production-backup-to-staging.sh", ["recovery_assurance"]],
  ["scripts/ops/f004-provider-adapter.sh", ["recovery_assurance"]],
  ["scripts/validate-f004-recovery-instrumentation.js", ["recovery_assurance"]],
  ["scripts/validate-f004-recovery-instrumentation-mutations.js", ["recovery_assurance"]],
  ["scripts/validate-f004-recovery-mutation-sharding.js", ["recovery_assurance"]],
]);

const RECOVERY_SENTINELS = new Map([
  ["scripts/ops/backup-production-data.sh", command(
    "recovery-sentinel-F004-M001",
    ["node", "scripts/validate-f004-recovery-instrumentation-mutations.js", "--shard-index", "0", "--shard-total", "53"],
    { mutationBearing: true, timeoutMs: 240_000 },
  )],
  ["scripts/ops/restore-production-backup-to-staging.sh", command(
    "recovery-sentinel-F004-M010",
    ["node", "scripts/validate-f004-recovery-instrumentation-mutations.js", "--shard-index", "9", "--shard-total", "53"],
    { mutationBearing: true, timeoutMs: 240_000 },
  )],
  ["scripts/ops/f004-provider-adapter.sh", command(
    "recovery-sentinel-F004-M042",
    ["node", "scripts/validate-f004-recovery-instrumentation-mutations.js", "--shard-index", "41", "--shard-total", "53"],
    { mutationBearing: true, timeoutMs: 240_000 },
  )],
]);
...
function orderedCommands(packIds, paths) {
  ...
  for (const packId of PACK_ORDER) {
    ...
    if (packId === "recovery_assurance") {
      for (const relative of [...paths].sort()) {
        const sentinel = RECOVERY_SENTINELS.get(relative);
        if (sentinel && !seen.has(sentinel.id)) {
...
export function localFocusedGatePolicyFingerprint() {
  const semantic = {
    ...
    recovery_sentinels: [...RECOVERY_SENTINELS].sort(([left], [right]) => left.localeCompare(right)),
