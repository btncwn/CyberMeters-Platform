...
const mkCase = (id, ws, domain_key, severity = "high") => {
  db.exec(`INSERT INTO managed_cases (id, workspace_id, domain_key, case_type, status, severity, domain, finding_id, created_at, updated_at)
           VALUES ('${id}','${ws}','${domain_key}','${domain_key === "brand_protection" ? "brand_case" : "asm_case"}','open','${severity}','x.example','f1',datetime('now'),datetime('now'))`);
...
// ── 3. First pass establishes the baseline and sends NOTHING ───────────────
{
  ...
  eq("first pass: no customer-visible notification", notifs("ws1").length, 0);
  const l = ledger("ws1");
  ok("first pass: suppression is recorded, not silent", l.some((d) => d.reason === "alert_baseline_established"));
  ok("first pass: the watermark now exists",
     db.prepare("SELECT COUNT(*) c FROM alert_activation WHERE workspace_id='ws1' AND domain_key='brand_protection'").get().c === 1);
  // The event is still persisted — evidence is not conditional on delivery.
  eq("first pass: the occurrence is still recorded as evidence", events("case_b").length, 1);
}
