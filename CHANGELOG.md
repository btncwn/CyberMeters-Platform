...
**Item 10 (Attack Surface depth) is ENGINEERING COMPLETE AND DEPLOYED**: P1 #326 · P2 #327 ·
P3 #330 · P4 #335 · P5 #336 · pagination corrective **#338**. Founder live acceptance is
**DEFERRED TO ITEM 14** — no production acceptance and no Item 14 PASS is claimed by this
entry.

- **Release identity:** deployed main SHA `b4fd347c53a9bd2acbaff2b87a17d567f19a3d2b`
  (merge of PR #338, reviewed head `f330511b`, parents `26565746` + `f330511b`; exact-main
  validate/sast/Pages all green). Release tag `v2026.07.27-3` on the deployed commit.
- **Migration:** none in this cutover. Migration `102` remains applied and unchanged.
- **scan-api (`cybermeters-platform`):** production Worker Version ID
  `8d4e36f1-c193-48f4-a0e5-dac05e8807f8`, deploy message
  `"Item 10 P5 pagination corrective PR #338 cutover b4fd347c…"`; **rollback:**
  `5624f0a5-151f-4c8e-a6cd-ab00959cad05` (the `-2` cutover, Cloudflare-confirmed live
  immediately before this deploy). **email-ingest: not deployed** (unchanged on
  `f4423b41…`).
- **What the corrective fixes in production:** the domain-level ASM assurance projection is
  now fed by one independent workspace-scoped bounded query
  (`LIMIT 500`, `COUNT(*) OVER ()` total) instead of the request's own paginated/filtered
  rows; the response carries explicit
  `attack_surface_assurance_coverage { returned, total, bound, truncated, status,
  customer_message }`; truncation is stated, never silent; list rows no longer duplicate
  the projection; the asset-detail endpoint keeps its asset-scoped projection and the
  `asset.attack_surface_assurance` compatibility alias; the canonical presentation engine
  is byte-unchanged.
- **Production proof (side-effect-safe):** both scan-api hosts
  (`api.cybermeters.com` and `cybermeters-platform.ttrnn47.workers.dev`) serve the same new
  deployment id `8d4e36f1…`; `/ready` reports `d1:true, r2:true`; `/api/workspaces`,
  `/api/scans` and `/api/workspaces/:id/assets` all return `401` (auth gates intact).
  Read-only D1 confirms the parity inputs: the largest workspace still holds exactly
  **238 assets** — the deployed code's coverage for it is
  `returned: 238, total: 238, bound: 500, truncated: false` by construction.
- **Proof boundary, stated honestly:** the authenticated parity behaviours — identical
  top-level projection across `?limit=1` / default / `?status=active`, confirmed-removed
  evidence surviving an active-only filter, no per-row duplication, asset-detail alias, and
  the **83.4 % response-size reduction** (238-asset response 1,769,693 → 293,273 bytes) —
  are proven by the corrective fixtures running the **real route handler** against a seeded
  238-asset workspace plus a genuine 501-asset truncation case (33/33 assertions; 4/4
  mutants killed). No founder session was available at deploy time, so authenticated live
  smoke on the production workspace remains a founder-session action inside Item 14. A
  `401` proves route liveness only, not the authenticated workflow.
- **Lifecycle clock at this deploy:** `asset_lifecycle_observations` and
  `attack_surface_signal_observations` still hold **0 rows** — no completed post-102 scan
  has written yet. Each asset's `confirmed_removed` window starts at its first qualifying
  post-deploy complete observation.

## v2026.07.27-2 — Migration 102 + Item 10 P3–P5 activation + SPF CIDR P1 cutover — deployed 2026-07-27; **DEPLOYED · NOT LIVE-ACCEPTED · CORRECTIVE BLOCKER CLOSED BY v2026.07.27-3**
...
### KNOWN CORRECTIVE BLOCKER — P5 domain projection page-coupled and duplicated — **CLOSED by PR #338 / `v2026.07.27-3` above**
...
Rolling back would also remove the working P3–P5/SPF fixes. The required focused corrective
PR is **#338**, deployed as **`v2026.07.27-3`** (see the entry above); with it Item 10 is
recorded ENGINEERING COMPLETE AND DEPLOYED, and CT-R1 is unblocked.
...
- **What shipped (behaviour-neutral by contract):** `execution_context` (`queue | cron | waituntil`, unknown → null) threaded from all three dispatch sites into `runScanEngine`; a guarded observational `onStart(capMs)` hook on `raceModuleDeadline` records the EXACT applied cap (incl. `hardMs` narrowing) as `allocated_ms`; engine-observed `wall_ms` for the race-bounded modules (takeover, exposure, phase-5 trio shares one race → shared allocation/wall, cloud storage); `timeout_source` vocabulary `module_race | launch_gate | per_fetch | null`; finalisation wall time as a D1-only `scan_finalisation` pseudo-row in the EXISTING 078 columns (excluded from `TELEMETRY_TRACKED_MODULES`, purged with the scan, invisible to every customer surface); `outbound_calls` populated ONLY for `cve_intelligence` (`technologies_checked` is an exact 1:1 NVD-request counter) — all other modules stay null, never estimated; a versioned `execution_diagnostics` block (`scan-exec-diag-v1`) on completed AND failed R2 reports with ZERO runtime consumers. No timeout, budget, ordering, fetch-signal, retry, scan_quality or customer-surface change.
- **Validation:** `validate-scan-telemetry.js` extended to 77 assertions incl. direction mutations (onStart reports `min(hardMs, remaining)` — not a re-computed remaining; exhausted budget never launches the thunk; `per_fetch` derivation vs explicit source; diagnostics write-only — exactly two engine references, both writes; `buildScanQuality` never reads diagnostics). All 21 validators importing the changed engines passed UNMODIFIED; regression fixtures 220/220; CI green on the exact head SHA before merge.
- **Deploy negative-effects proof (before any scan):** zero new scans/audits/findings/notification_events/cases/telemetry rows in the deploy window; zero active rows; no config drift.
- **MANUAL LIVE ACCEPTANCE — PASS (founder-run, 22 Jul ~16:0xZ, blackbullbarbers.co.uk):** first POST 202 `{status:"queued", scan_quality:null}` admitted `scan_d7ac943f-8ce5-4733-8d81-fa6f409beb5f`; immediate duplicate → byte-exact canonical 409 `{"error":"A scan is already active for this domain.","code":"active_scan_exists"}` with ZERO row/audit/R2/engine/notification side effects (1 scan row, 1 `scan_requested`, 1 `scan_started`). Lifecycle queued→running→`completed` (score 95, earned `scan_quality: partial`). Terminal R2 carries `execution_diagnostics.version = "scan-exec-diag-v1"`, `execution_context: "queue"`, `deadline_budget_ms: 21000`, `engine_wall_ms: 25175`, 14-module array with full field shape, zero `timeout_source` vocabulary violations, `outbound_calls` null everywhere it isn't reliably known. D1 holds exactly ONE `scan_finalisation` pseudo-row (`duration_ms` 888) — 15 telemetry rows = 14 modules + finalisation; the R2 diagnostics module count is 14 (pseudo-row excluded). D1 == R2 terminal (completed/95/partial). Events correctly suppressed by the PRE-EXISTING partial gate (0 asset_events — unchanged behaviour); 0 cases from the scan (the two same-day cases predate the deploy at 01:07Z/11:54Z); no stale active rows.
- **First live telemetry finding (the evidence this PR exists to produce):** in the acceptance scan, `headers` consumed **20,000 ms of the 21,000 ms budget**, and all six later modules (takeover, exposure, CVE, KEV, email-intel, cloud storage) were refused at the launch gate — `allocated_ms: 0, timeout_source: "launch_gate"` — driving the `partial`. Per-run starvation attribution is now readable from any single report; this run's driver was headers, not ssl.
- **SCHEDULED (CRON) LIVE ACCEPTANCE — PASS (22 Jul, recorded by founder authority):** the 19:00Z scheduled scan `scan_c397a9e5-8ae8-4e0c-b452-841e60389f85` (created 2026-07-22 19:00:37Z) reached `completed` / `partial` / score 95 with `execution_diagnostics.version = "scan-exec-diag-v1"` and **`execution_context: "cron"`** — proving the context split against the manual scan's `"queue"`. Diagnostics schema parity with the manual acceptance: `deadline_budget_ms: 21000`, `engine_wall_ms: 22264` (non-negative, populated), 14-module array + exactly one D1 `scan_finalisation` pseudo-row. D1 and R2 terminal state agree; **no Queue dispatch and no `queue_claimed` stage occurred for the scheduled path** (cron execution behaviour unchanged, as designed); no duplicate lifecycle effects; the only notification was the scan's own ordinary `scan_completed` informational. `engine_wall_ms` 22,264 > the 21,000 budget is live evidence of the already-documented abandonment-without-abort design (`raceModuleDeadline` abandons, per-fetch timeouts run on) — not a new discovery; the abort-propagation fix belongs to the separately gated deadline-replacement work. `outbound_calls` remains an unmeasured null stub except the CVE 1:1 counter, as shipped.
- **Status: PR-A1 Scan Execution Timing Telemetry — declared MERGED / DEPLOYED / LIVE-ACCEPTED** (manual scan `scan_d7ac943f-8ce5-4733-8d81-fa6f409beb5f` = queue context; scheduled scan `scan_c397a9e5-8ae8-4e0c-b452-841e60389f85` = cron context). **Migration: none — 100 remains unused. Residual (explicit):** partial scans can still complete with "score 95 / excellent"-style wording without explicitly disclosing degraded monitoring to the customer — the degraded-monitoring customer wording is a separately tracked backlog item (item 5); the 21s budget and the global complete-gate remain by design (deadline replacement = PR-B; per-signal completeness = PR-C/D — all separately founder-gated); public beta remains blocked.

## v2026.07.22-7 — PR-3B: durable Queue dispatch ENABLED — deployed 2026-07-22; **PR-3 LIVE-ACCEPTED**

- **Release tag:** `v2026.07.22-7` (deployed commit = main HEAD `518af83`, PR #278). **Live Worker Version ID:** `793c040c-a36e-41b1-9aeb-d8833d5d1eba` (deployed 14:24:28.347Z CF-authoritative; custom domain + workers.dev parity, cache-busted). **Rollback:** `b933520e-42a1-4fa2-9791-18763b915938` (v2026.07.22-6) — canonical rollback is flag-back (`SCAN_DISPATCH_MODE="waituntil"`, moving the E2d pin in the same commit) + redeploy; the waitUntil branch remains in the deployed code. **Migration:** none (099 unchanged; **100 unused**).
- **Config-only cutover:** `SCAN_DISPATCH_MODE` `"waituntil"` → `"queue"` — one runtime line. Second file in the diff (disclosed in PR #278): the validator's own flag pin (E2d) moved in lockstep to pin the flag ON, so a silent config revert/removed flag/typo reddens CI. Manual scans now admit as `queued` and execute in the Queue consumer (full invocation lifetime), replacing the post-response ~30s waitUntil path — the root cause of the 22 Jul stuck-`running` incident class.
- **PR-3 FINAL LIVE ACCEPTANCE — PASS (founder-run, 22 Jul ~14:35Z, Turhan Workspace + blackbullbarbers.co.uk):** first POST 202 `{status:"queued", scan_quality:null}` admitted `scan_1f391af3-8154-462c-b969-55789e8c86cf`; immediate duplicate POST → byte-exact canonical 409 `{"error":"A scan is already active for this domain.","code":"active_scan_exists"}`. Read-only verification of all 18 gates: exactly one new row (132→133); queued→running→`completed` (score 95/excellent, earned `scan_quality: partial`); ~2.8s queued dwell (row 14:35:19 → engine `started_at` 14:35:21.792 — Queue delivery + consumer claim; in queue mode only the consumer claim CAS performs queued→running and only the consumer can start the engine, and `scan_started` is consumer-written at claim, CI-pinned); exactly one `scan_requested` (14:35:19) + one `scan_started` (14:35:21) + one `scan_completed` (14:36:35); duplicate created ZERO rows/audits/R2/engine effects (workspace audits 246→249 = the three lifecycle events only; zero `scan_dispatch_*`); exactly one engine execution (zero duplicate telemetry modules, one 5-finding set); D1 == R2 terminal (completed/95/partial); notifications +2 both the scan's own (its `scan_completed` + one CE-readiness lifecycle alert, no duplicates); cases unchanged; zero cross-tenant audits since deploy; zero stale queued/running/retrying rows; dispatch queue drained; DLQ untouched (Last-Modified still its 13:56:21Z creation instant, 0 producers/0 consumers).
- **PR-3 (durable Queue dispatch) declared MERGED / DEPLOYED / LIVE-ACCEPTED** under the founder authority of 22 Jul, spanning PR #277 (`b60eec5`, PR-3A implementation behind disabled flag, v2026.07.22-6) and PR #278 (`518af83`, this cutover). Queue resources: `cybermeters-scan-dispatch` (id `b99c1e3343ac4a10b1b8e271d11a3044`) + `cybermeters-scan-dlq` (id `e196b3697e7941b2b5e00845dbe445f1`), created 13:56Z before the PR-3A deploy.
- **Residual risks (explicit):** the 21s engine deadline budget remains (coverage characteristics unchanged — deadline replacement is a separate backlog item); no crashed-running auto-retry (a lease/reclaim design needs the dedupe hardening priced in the PR-3 design package; PR-1 recovery terminalises honestly instead); scheduled scans remain on the cron path by design; the DLQ has no consumer by design (recovery converges dead-lettered rows); **public beta remains blocked** pending the remaining readiness work.

## v2026.07.22-6 — PR-3A: durable Queue dispatch behind disabled cutover flag — deployed 2026-07-22 (inert); **FLAG OFF ACCEPTED**

- **Release tag:** `v2026.07.22-6` (deployed commit = main HEAD `b60eec5`, PR #277 squash of head `feaae6e6`). **Live Worker Version ID:** `b933520e-42a1-4fa2-9791-18763b915938` (deployed 13:58:35.907Z CF-authoritative; both hosts verified). **Rollback:** `feef8306-cf40-4449-bed6-244c860b0cc3` (v2026.07.22-5). **Migration:** none (100 deliberately unused — `queued` was already guarded by 099, and Queue delivery metadata is authoritative for retry state, so no new columns earned their keep).
- **What shipped (inert until the -7 cutover):** `engines/scan-dispatch.js` — v1 message contract (only `scan_id` execution-authoritative; advisory workspace/domain/domain_id cross-checked against the D1 re-read, fail-closed `scan_dispatch_mismatch` on drift); producer dispatch setup (queued R2 placeholder + `SCAN_QUEUE.send` as ONE compensation boundary → CAS queued→failed with `scan_quality` NULL, `scan_dispatch_failed` audit, customer-safe 503; failed compensation leaves the recoverable queued row for PR-1); consumer claim CAS `queued`→`running` (exactly-once execution; duplicate/terminal/post-recovery deliveries ack as no-ops), `scan_started` moved to claim time, `queue_claimed` heartbeat, engine AWAITED in the queue invocation, engine failures acked never queue-retried (the canonical finalize latch owns terminals). `retrying` is never persisted in v1. `RECOVERY_ACTIVE_STATUSES` widened to `["queued","running"]` (own set, never imported from admission) with `status_before` in the recovery audit. Safety argument: every non-idempotent side-effect writer runs strictly AFTER terminal finalisation, and the claim CAS refuses non-queued rows — at-least-once delivery structurally cannot double-run them.
- **Validation:** new CI gate `validate-scan-queue-dispatch.js` (48 assertions, 5 layers incl. REAL-route producer in both modes over real SQLite + the real 099 index, and an SQL-faithful claim-CAS stub); **9/9 mutation directions caught**; full local gate green (admission 44/44, recovery, stabilization, presentation-parity, eight-domain, M5.c/M5.d, scan-deadline 90/90, subrequest 71/71, regression 220/220, pipeline 42/42).
- **Inert + legacy acceptance (founder-run, ~14:07Z):** deploy produced ZERO side effects (all baselines byte-identical); legacy scan `scan_fcb812be-29fd-4cd0-b423-6a0bcd953806` → 202 `running` → `completed` 87/good, quality `partial`, on the waitUntil path with the queue provably untouched; duplicate → canonical 409 with zero side effects. **PR-3A declared MERGED / QUEUE RESOURCES CREATED / DEPLOYED INERT / FLAG OFF ACCEPTED.**

## v2026.07.22-5 — PR-2c: canonical `active_scan_exists` 409 serializes exactly `{ error, code }` — deployed 2026-07-22; **PR-2 LIVE-ACCEPTED**

- **Release tag:** `v2026.07.22-5` (deployed commit = main HEAD `62575a8`, PR #275). **Live Worker Version ID:** `feef8306-cf40-4449-bed6-244c860b0cc3` (deployed 12:19:49Z CF edge; custom domain + workers.dev parity). **Rollback:** `00d8082d-66a1-4d03-a88b-7dbe1dcd4722` (v2026.07.22-4). **Migration:** none (099 unchanged).
- **Root cause (found by the first founder-run live acceptance, which correctly FAILED):** `normalizeApiResponseData` — the single error-shape choke point behind `json()` — injected its `STATUS_MESSAGE[409]` fallback into the frozen conflict body, adding a third `message` key the approved two-key contract forbids. CI missed it because the route harness stubbed `json()`; the harness now serializes every response through the REAL middleware (permanent).
- **Fix (deliberately narrow):** `CONTRACT_EXACT_CODES = Set(["active_scan_exists"])` short-circuits only the message fallback for bodies that ARE the complete approved contract. Unrelated 409s/5xx keep full `{ error, code, message }` decoration; `detail`/`stack` stripping still applies to exact-contract codes. Validator 44 assertions incl. new Layer F; mutation (dropping the guard) reddens E2+F1.
- **PR-2 FINAL LIVE ACCEPTANCE — PASS (founder-run, 22 Jul ~12:48Z, Turhan Workspace + blackbullbarbers.co.uk):** first POST 202 admitted `scan_1916d8bf-8bde-446b-990a-cf513e6f7915`; immediate duplicate POST 409 with body exactly `{"error":"A scan is already active for this domain.","code":"active_scan_exists"}` (keys `["code","error"]`). Read-only production verification: exactly one new scan row (130→131; workspace/domain 41→42); admitted scan carries exactly one `scan_requested`, one `scan_started`, one `scan_completed`; rejected request created ZERO rows/audits/R2 objects/engine starts; the only notification delta is the admitted scan's own `scan_completed`; cases unchanged; both 099 indexes present; no duplicate active group. **Admitted scan reached `completed` with `scan_quality: complete`.**
- **PR-2 (atomic active-scan admission guard) declared MERGED / MIGRATION APPLIED / DEPLOYED / LIVE-ACCEPTED** under the founder authority of 22 Jul, spanning: PR #273 (`5f7c5f3`, mig 099 + guard, v2026.07.22-3), PR #274 (`6653c14`, audit-after-admission, v2026.07.22-4), PR #275 (`62575a8`, canonical 409 shape, this release).
- **Residual risk:** manual scan execution still runs on the ~30s post-response `waitUntil` path until **PR-3 (durable Queue dispatch)** — the next task in the scan-lifecycle sequence. A stuck-`running` scan blocks re-admission for its (workspace, domain) until PR-1's hourly recovery fails it (bounded; the 409 says so honestly). Public beta remains blocked.

## v2026.07.22-4 — PR-2b: `scan_requested` written only after successful admission — deployed 2026-07-22 (code-only, no migration)

- **Release tag:** `v2026.07.22-4` (deployed commit = main HEAD `6653c14`, PR #274). **Live Worker Version ID:** `00d8082d-66a1-4d03-a88b-7dbe1dcd4722` (deployed 11:22:07Z CF edge; both-host parity). **Rollback:** `ed160106-0b8d-4795-afda-c816b25c52cc` (v2026.07.22-3). **Migration:** none.
- **Defect (flagged proactively during the PR-2 release, founder ruled it a correction):** the manual route wrote the `scan_requested` lifecycle audit BEFORE the atomic INSERT, so a constraint-rejected duplicate left a misleading audit with no scan row. Canonical order now: authority checks → atomic INSERT → (success) audit + placeholder + engine; (conflict) canonical 409 with zero side effects; (other DB error) real failure, no lifecycle audit. Audit write non-fatal (matches `scan_started`) so an audit failure can never strand an admitted running scan. Scheduled path already post-insert (pinned).
- **Validation:** validator Layer E drives the REAL `scanRoutes` against a real SQLite DB (schema + all migrations, real 099 index) with a parked engine — 40 assertions; 3 mutation directions (audit-before-INSERT → E3/E6; audit-in-catch → E3; audit-suppressed → E1). Fixture dates computed from the real clock after the date-rot governance gate correctly rejected literal future dates.

## v2026.07.22-3 — PR-2: atomic active-scan admission guard (migration 099) — deployed 2026-07-22

- **Release tag:** `v2026.07.22-3` (deployed commit = main HEAD `5f7c5f3`, PR #273 — includes the founder pre-merge correction `e7a9efd`: customer-safe constant 409 body, read-side enrichment removed). **Live Worker Version ID:** `ed160106-0b8d-4795-afda-c816b25c52cc` (deployed 10:36Z CF edge). **Rollback:** `4518ed5a-ea2a-46fc-ac81-bb3807dc2027` (v2026.07.22-2).
- **Migration 099 applied to production D1 (before the dependent deploy):** two partial UNIQUE indexes — `scans(workspace_id, domain) WHERE status IN ('queued','running','retrying')` + defensive `scans(domain) WHERE … AND workspace_id IS NULL` (SQLite UNIQUE treats NULLs as distinct). `rows_written: 2`, applied exactly once; pre-checks proved 0 active rows and 0 duplicate groups (129 = 123 completed + 6 failed, all counts unchanged after). Index-only; rollback = `DROP INDEX` both.
- **Invariant (closes the 22 Jul three-concurrent-scans defect):** at most ONE active scan (`queued`|`running`|`retrying`) per (workspace, canonical domain), enforced by the DATABASE — the INSERT is the admission decision; no pre-insert SELECT/frontend state/in-memory lock is trusted. Key is the normalized `domain` STRING, not `domain_id` (domains rows are per-user; one domain legitimately exists under several domain_ids — mutation-proven A3/A12). `queued`/`retrying` covered before PR-3 ever writes them. Manual duplicate → constant customer-safe 409; scheduled conflict → eligibility-style skip, schedule stays due, retries next hourly tick.
- **Validation:** admission validator (real-SQLite over schema + all migrations; drift guard constant↔index WHERE; source contracts; race fixture) CI-wired; 7 mutation directions proven; frontend renders the canonical error with zero runtime change (7-test conflict suite); regression 220/220; tenant-isolation/authz green.

## v2026.07.22-2 — PR-1b: running/interrupted scans never carry earned quality + bounded 3-object R2 repair — deployed 2026-07-22 (code-only, no migration)

- **Release tag:** `v2026.07.22-2` (deployed commit = main HEAD `1f81bcc`, PR #271). Runtime delta vs `v2026.07.22-1` = exactly PR-1b: `routes/scans.js` (START placeholder `scan_quality: null` — it previously hardcoded `status:"complete"` on a RUNNING scan, a pre-existing honesty defect) + `engines/scan-recovery.js` (interrupted failed report explicitly sets `scan_quality: null` after its spread). Canonical invariant (founder decision 22 Jul): **a running or interrupted scan has not earned `complete` or `partial`; only legitimate terminal finalisation may persist an earned quality.**
- **Live Worker Version ID:** `4518ed5a-ea2a-46fc-ac81-bb3807dc2027` (deployed 03:20:42Z; verified at `GET /health` on BOTH hosts — custom domain + workers.dev parity). **Rollback:** `536ce9d1-51e3-4ab8-b896-a02a25aa6c32` (v2026.07.22-1).
- **Migration:** none. Consumer safety proven by discovery (every reader null-safe; nothing consumes the placeholder's `subrequest_budget`) + frontend vitest 441/441.
- **Bounded canonical R2 repair (founder-approved one-time):** `scripts/repair-interrupted-r2-quality-2026-07-22.js` — pinned to the exact three incident scan IDs; guards required D1 `failed`+`scan_quality IS NULL` AND R2 `failed`+`interrupted_before_finalisation`+non-null inherited quality; changed ONLY `scan_quality → null` (computed change-set proof); sha256 before/after per object; post-write verified; second run 3× NO-CHANGE (idempotent); refuses everything else. Repaired: `scan_eb0aba0c…` (`54141bfd…→bbf770da…`), `scan_32945fab…` (`bd1a9447…→ea1f8333…`), `scan_deb2244f…` (`889fd6d9…→99f4e300…`).
- **Contracts:** recovery validator fixtures 1h/12-complete/12-partial (inherited quality never survives); stabilization pins both nulls. Mutation-proven: removing the explicit null → 4 FAILs; reintroducing hardcoded `complete` → FAIL.
- **PR-1 LIVE ACCEPTANCE — PASS (04:00Z cron idempotency):** all three rows stayed `failed`+NULL, audit count stayed exactly 3 (no duplicates), R2 hashes byte-identical post-tick, `running_now=0`, zero alerts/cases/findings/scores. **PR-1 declared MERGED / DEPLOYED / LIVE-ACCEPTED** under the founder authority of 22 Jul. The wider manual-scan P1 is NOT complete: PR-2 (atomic active-scan admission guard, mig 099) and PR-3 (durable Queue dispatch) remain outstanding; manual scans still run on the ~30s post-response `waitUntil` path until PR-3.
- **Independent post-tick verification pass (evidence collected 04:02:39–04:06Z, window gated on Cloudflare edge `Date` — a local background-task clock ran ~27 min fast during acceptance, so wall-clock alone was not trusted):** every Phase-G gate re-proven against production after the 04:00Z tick. D1: all three rows `failed` + `scan_quality` NULL. Audit: `scan_interrupted_recovered` count exactly 3 — 1 per scan, original timestamps unchanged (03:00:03/03:00:04Z), no duplicates, and no scan outside the pinned three has ever carried this event (no unrelated transition). Zero across all read surfaces: `findings`/`asset_alert_records`/`managed_cases` by scan = 0; incident-workspace `notification_events`/`managed_case_events`/`managed_cases`/`alert_deliveries` since 22 Jul 00:00Z = 0. R2: all three reports canonical (`failed` · `interrupted_before_finalisation` · `scan_quality: null` · `findings: []` · no score/rating) and sha256 **byte-identical across the tick** — `bbf770da97ad628b6a9cf124a209ecbe2909e93b5cf93529978adbcc36ab91a6` (eb0aba0c), `ea1f8333bf18d59bef714c351584e3252d82084570d22364702dbcf944dbb6bd` (32945fab), `99f4e30005cf0767bced56af514c31daaf3a79741dbb270601a9b14b990fec3b` (deb2244f). Repair script re-run post-tick: `repaired=0 unchanged=3 refused=0`, exit 0 (idempotent). The earlier wrong-path repair invocation performed no write — it fails on its first call, a read-only D1 SELECT, and the script's single write site sits behind all guards.

## v2026.07.22-1 — PR-1: interrupted-scan terminal recovery (canonical cron repair; read-only GETs) — deployed 2026-07-22 (code-only, no migration)

- **Release tag:** `v2026.07.22-1` (deployed commit = main HEAD `5ac30f0`, PR #270; guard re-pins in #270 follow-ups `e143a8b`-series). Closes the permanent-`running` class from the 22 Jul manual-scan incident: manual scans run in post-response `ctx.waitUntil`, which the runtime cancels ~30 s after the response (empirically proven — three scans froze at exactly +30 s); a scan killed mid-flight never finalises, and the pre-existing reconcilers (R2-terminal → D1 only) could never repair it.
- **Live Worker Version ID:** `536ce9d1-51e3-4ab8-b896-a02a25aa6c32` (deployed 02:49:15Z). **Rollback:** `ecd03d0a-bbcd-42d9-95ad-bcf01e632028` (v2026.07.21-5).
- **What shipped:** ONE canonical recovery brain (`engines/scan-recovery.js`), cron-only (`interrupted_scan_recovery` task): eligibility = active status AND age > 30 min AND heartbeat stale > 10 min AND R2 absent-or-running placeholder; class 1 converges D1 from a terminal R2 report (never overwrites); class 2 marks R2 then D1 `failed` (`AND status IN` guard — a racing finalisation always wins) with reason `interrupted_before_finalisation`, one append-only audit event, NO alerts/cases/findings/scores. GET `/api/scans` list+detail became READ-ONLY (their embedded D1 UPDATEs removed; display-only correction kept). Bounded 25/tick, idempotent.
- **Migration:** none. Preflight proved the candidate set was exactly the three known incident scans (ages 91–99 min, heartbeats stale, R2 running placeholders).
- **First-cron proof (03:00:03Z):** all three rows → `failed` + `scan_quality NULL` + correct tenant attribution; exactly 3 system audit events; zero alerts/cases/findings; unrelated scans untouched. One live-acceptance gate initially FAILED — the R2 failed reports inherited `scan_quality.status="complete"` from the START placeholder — closed by v2026.07.22-2 above.
- **Validation:** `validate-interrupted-scan-recovery.js` (stub D1/R2, SQL-faithful guards) CI-wired; 5 mutation directions proven; three stale guard pins re-pointed at the canonical site (parity, stabilization Episode 5B → five founder contracts, pipeline exact task-set).

## v2026.07.21-5 — Chronic-partial reliability fix: SSL runs CT lookup concurrently with reachability (PR #263) — deployed 2026-07-21 (code-only, no migration)

- **Release tag:** `v2026.07.21-5` (deployed commit = main HEAD `506f364`). Runtime delta vs the previous deploy (`532f391`, v2026.07.21-4) verified pre-deploy as **exactly** PR #263 (`e9e169a`) — one `workers/` file, `engines/ssl-scan.js`. Every other commit in the range (#259–#265) is docs-only (changelog, Detection-Depth governance, DMARCbis/Brand-IDN design). **No unrelated runtime delta.**
- **Live Worker Version ID:** `ecd03d0a-bbcd-42d9-95ad-bcf01e632028` (verified at `GET /health`, 5/5 cache-busted probes after propagation).
- **Rollback Worker Version ID:** `d1ad62b8-3d46-40f3-896a-a2d510176d25` (v2026.07.21-4 — confirmed the live pre-deploy version via `wrangler deployments list` and cache-busted `/health`).
- **Migration:** none — verified (`git diff 532f391..506f364 -- database/` is empty). Latest applied remains `098-related-changes.sql`. **No Stripe / DNS / legal / DMARC / SPF-config / products / prices / secrets / webhooks / Pages / production data changed.**
- **What shipped (PR #263):** `runSslModule` ran its HTTPS reachability probes and its Certificate-Transparency cert lookup (crt.sh 8s + certspotter 8s fallback) **sequentially**, so the module's wall-time was the **sum** of both chains — up to ~28s on a slow-crt.sh day, exceeding the 21s scan deadline and starving the deadline-gated modules (chronic `partial`). The CT lookup is extracted into `resolveCertificateTransparency()` and runs **concurrently** with the reachability probes; reachability code is byte-identical, all 11 `cert_*` fields preserved, failures best-effort. SSL wall-time → **max(reachability, CT)** instead of sum.
- **Pre-deploy validation (all on `506f364`):** `validate-ssl-concurrent-ct.js` 12/12 (mutation-verified — reverting to a sequential CT reddens the max-not-sum assertion); scan-deadline/budget/engine-counter/hard50/reserved, probe-honesty, posture-events(+resolution), SPF resolver+corroboration, m6-b1, identity green; the six security suites green; regression 220/220; Worker syntax; wrangler dry-run; ci-governance; semgrep 0 findings; `git diff --check` clean.
- **Production smoke (post-deploy):** `GET /health` → `ecd03d0a` (5/5 cache-busted); `billing/subscription` 401, `workspaces` 401, `auth/login` 400, public `billing/plans` 200 — no regression.
- **Live exit-gate — RELIABILITY IMPROVED, RESIDUAL BLOCKER REMAINS.** Measured live external-call latency for the anchor domain (Turhan Workspace `workspace_2aaf14fb` → `dom_db28b2e8`): reachability fast (~0.5s total); **crt.sh currently DOWN** (cert query times out ~8s, wildcard ~12s, HTTP 000); certspotter fast (~0.39s). SSL wall-time drops from the observed **28s → ~8.4s** (sum→max) — SSL is no longer the bottleneck. **Residual:** `subdomains` still hits its constant ~12s crt.sh-wildcard timeout (57% of budget) and `headers` can take up to ~20s — either can still push a scan `partial`. The next workspace scan is **daily** (next `2026-07-22T19:00`) and could not be triggered autonomously without founder auth or mutating `scheduled_scans` (production data — not done), so the actual post-fix `complete`/`partial` result is **pending** the next cron or a founder-triggered scan. Authoritative customer posture stayed on the last complete scan (unchanged). **SPF live acceptance is NOT yet unblocked**; the definitive remedy remains **per-signal evidence completeness** (Detection Depth Law). Details in `docs/CHRONIC-PARTIAL-SCAN-ROOT-CAUSE.md`.
- **Rollback:** `npx wrangler rollback` to `d1ad62b8` restores v2026.07.21-4. Not triggered — no health/route regression; the change is additive and evidence-preserving.

## v2026.07.21-4 — SPF include-aware authorisation-change detection + DMARC-RUA corroboration (PR #251/#252) — deployed 2026-07-21 (code-only, no migration)

- **Release tag:** `v2026.07.21-4` (deployed commit = main HEAD `532f391`). Runtime delta vs the previous deploy (`7374c41`, v2026.07.21-3) verified pre-deploy as **exactly** PR #251 (`0d02d89`) + PR #252 (`25c5c0d`) — 7 `workers/` files: `engines/spf-resolver.js` (new), `engines/spf-corroboration.js` (new), `engines/email-scan.js`, `engines/posture-events.js`, `engines/scan-engine.js`, `engines/remediation-registry.js`, `engines/related-changes-adapter.js`. **No unrelated runtime delta** — none of the other commits in the range (#240–249, #253–258) touch `workers/`, and none of stripe/billing/auth/webhook/index.js appear in the diff.
- **Live Worker Version ID:** `d1ad62b8-3d46-40f3-896a-a2d510176d25` (verified at `GET /health`, 3/3 cache-busted probes).
- **Rollback Worker Version ID:** `bd33b028-5209-427b-873a-0779e91350ff` (v2026.07.21-3 — confirmed the live pre-deploy version via `wrangler deployments list` AND cache-busted `/health`).
- **Migration:** none — verified (`git diff 7374c41..532f391 -- database/` is empty). Latest applied remains `098-related-changes.sql`. **No Stripe prices, products, secrets, webhooks, DNS, legal content, or Pages changed.** Pages not deployed (no runtime frontend change in the delta).
- **What shipped (PR #251, PR-A):** recursive statically-resolvable SPF PASS-authorisation resolver (`include:`/`redirect=`/`ip4`/`ip6`/`a`/`mx` + dual-CIDR, recursive resolution, cycle detection, 10-DNS-lookup + 2-void-lookup limits, multiple-SPF → PermError, **TempError fail-honest** with a null set, CIDR containment v4+v6). New `email_spf_authorization_changed` event fires **only when both compared snapshots resolved to status `complete`**; existing root-string `email_spf_changed` (Q729) is preserved unchanged. Additive to the R2 `email_security.spf` snapshot.
- **What shipped (PR #252, PR-B):** DMARC-RUA corroboration (`engines/spf-corroboration.js`) — the RUA report's own SPF result is authoritative; a firm "unauthorised source" finding needs source_ip + observed `fail` + a **complete** resolution + IP-not-contained + unambiguous attribution; `~all`/`?all`/`+all` → cautious wording; partial/temperror/permerror → limited "comparison unavailable" wording; routed through the canonical `emitManagedAlert` with per-(domain, source_ip) dedupe. New remediation `email.spf.unauthorised_source` (manual_attestation).
- **Pre-deploy validation (all on `532f391`):** `validate-spf-resolver` 49/49, `validate-spf-corroboration` 31/31, posture-events + posture-events-resolution + dmarc-dns-evidence-pr2 + m6-b1-related-changes + exposure-feed + all six remediation-registry validators green; the six security suites green (no drift); regression fixtures 220/220; Worker syntax OK; `wrangler deploy --dry-run` OK; ci-governance green; semgrep on the 7 SPF files 0 findings; `git diff --check` clean. Three load-bearing safety guards re-confirmed by mutation on `532f391`: the both-`complete` diff gate (defeat → false-alert red), CIDR containment (loosen → membership red), the corroboration complete-gate (defeat → partial-tier red).
- **Explicit safety confirmations:** root-only `email_spf_changed` behaviour intact (Q729 non-regression covered in the resolver change-detection suite); incomplete/TempError resolution does not emit a false authorisation-set diff (mutation-proven); recursive/cyclic/lookup-limit cases fail honestly (PermError); no migration; no Stripe secret/config change; rollback (`bd33b028`) confirmed viable.
- **Production smoke (post-deploy):** `GET /health` → `d1ad62b8` (3/3 cache-busted); `billing/subscription` 401, `workspaces` 401 (auth-gated, live), `auth/login` POST `{}` → 400 (validation, live), public `billing/plans` → 200. No regression in health/readiness or unrelated critical routes.
- **Controlled acceptance — BLOCKED (environmental, not a code/deploy defect).** Passive verification proved the anchor: **Turhan Workspace** `workspace_2aaf14fb-cf28-49d4-bde0-5eb645eda1a8` → domain_id `dom_db28b2e8-29c3-4e1b-b630-676fd347a7f0` (not deleted); live root SPF `v=spf1 include:secureserver.net -all` (Cloudflare + Google DoH, matches expected); no active 24-hour `email_spf*` dedupe; SPF code present in the deployed commit. **Blocker:** `assessTimelineComparison` (timeline-trust) requires both the current AND previous scan to be `scan_quality='complete'`, and `blackbullbarbers.co.uk` has produced only `partial` scans since 2026-07-18 (7 modules incomplete despite a 925/1000 subrequest-budget headroom; last `complete` scan `scan_ccc13a02`, 2026-07-18 11:19). With a partial current scan, `recordPostureEvents` returns early, so **neither `email_spf_changed` (Test 1) nor `email_spf_authorization_changed` (Test 2) would fire even after a real DNS change** — a production DNS change was therefore NOT requested (no evidence payoff, real risk). Restoring complete scans for this domain is scan-engine work, out of scope for this SPF-only release.
- **Acceptance status:** root-change acceptance BLOCKED (partial scans); **include-aware live acceptance PENDING — safe controlled child-include fixture required** (no founder-controlled isolated `include:` target exists that can safely add/remove a documentation CIDR without touching GoDaddy-controlled `secureserver.net`); RUA corroboration live acceptance PENDING (no genuine matching RUA fail-source injected or fabricated). A duplicate-record note: `blackbullbarbers.co.uk` has 5 domain rows across separate workspaces (Turhan / Berbero's / Black Bull Barbers / BBB / deneme[deleted]); the anchor is unambiguous per (workspace_id, domain_id), and any evidence read must be filtered to the exact pair.
- **Rollback:** `npx wrangler rollback` to `bd33b028-5209-427b-873a-0779e91350ff` restores v2026.07.21-3. Not triggered — no health/route regression; the SPF code is additive and fail-honest.

## v2026.07.21-3 — M7 billing-webhook hygiene (status lockstep + Basil period capture) + deferred A6 PDF-label deploy — deployed 2026-07-21 (code-only, no migration)

- **Release tag:** `v2026.07.21-3` (deployed commit = main HEAD `7374c41`, squash-merge of PR #239; the deploy also carries PR #238, merged earlier with its Worker deploy deliberately deferred).
- **Live Worker Version ID:** `bd33b028-5209-427b-873a-0779e91350ff` (verified at `GET /health`, 3/3 cache-busted probes).
- **Rollback Worker Version ID:** `a057a603-cb5c-4679-a4be-823cdb819c04` (v2026.07.21-2 — confirmed live pre-deploy via `wrangler deployments list` AND cache-busted `/health`; carries the founder's sandbox Stripe secret configuration, so rollback restores the full B2/B3-accepted state).
- **Migration:** none. Latest applied remains `098-related-changes.sql`. **No Stripe prices, products, secrets or webhook endpoints changed.** Runtime delta `a057a603 (v2026.07.21-2) → 7374c41` verified pre-deploy as exactly three files: `engines/stripe.js` (#239) + `engines/pdf.js`, `engines/related-changes-rules.js` (#238).
- **#239 defect 1 — legacy `status` column stale (founder B2/B3 D1 evidence: `subscription_status='active'` + `status='trialing'` on the paid test row):** webhook writers set only `subscription_status`, leaving the legacy `status` column at its old value. Entitlement was never wrong (the canonical resolver reads `subscription_status || status`), but any direct reader of the raw column was misled. `subscription_status` stays canonical; `status` is now written in LOCKSTEP at all six webhook write sites (upsert UPDATE, trial-row P2-D UPDATE, INSERT, subscription.deleted, invoice.payment_failed, invoice.payment_succeeded — whose CASE also re-syncs a historically stale value on the next event). A validator-level divergence guard asserts no row can end with `status != subscription_status`.
- **#239 defect 2 — `current_period_end` persisted NULL:** Stripe API 2025-03-31 (Basil) moved `current_period_start/end` off the subscription object onto the subscription items, and the Worker pins no `Stripe-Version` — the 2026-created account's payloads carry no top-level period, so `NULL` was written. New `getStripeSubscriptionPeriod()` reads top-level first (older API versions), then the BASE plan item (same mapped-price base-item selection as plan resolution — never blindly `items[0]`); used by the upsert and deleted handlers. The forward-only stale-event guard on `current_period_end` is unchanged.
- **#238 (deploy of previously merged change):** Executive PDF `RELATED_CHANGE_LABELS` cert halves now say "a certificate signal" (aligned with frontend `RULE_LABELS`, PR #237 — A6 P3 title↔subtype semantic alignment); rules-engine title/summary metadata aligned to prevent drift. Presentation-only at render time; `rule_id` is the frozen fact, snapshots untouched.
- **Validation (all on `7374c41` or its PR head):** `validate-stripe-events.js` extended 20 → **39 checks** — lockstep at every event type, Basil single-item + multi-item (base-item period, not the addon's), the exact founder-evidence row shape (June trial row + paid checkout → ONE row upgraded in place, both columns `active`, period repaired from NULL), the workspace-found trial-branch path, global divergence guard; **5 mutations** (each lockstep site + the Basil fallback removed) each fail the suite. Billing-lifecycle 23/23, Q3 idempotency 37/37, email-entitlement, pricing-policy-parity, regression fixtures 220/220, FE suite + build, Worker syntax, wrangler dry-run, `git diff --check` — green; full validator sweep green in CI on both PRs.
- **Production proof:** `GET /health` → `bd33b028`, 3/3 cache-busted. The stale prod row itself (`sub_1TvaUQ…` workspace) self-repairs on its next `customer.subscription.*` event; the founder's planned pre-go-live D1 reset supersedes manual repair — no production data was mutated by this deploy.
- **M7 status:** unchanged — B2/B3 founder-accepted (sandbox); LIVE cutover under the sole-trader entity still pending per `docs/M7-STRIPE-CUTOVER-ACTIONSHEET.md`.

## v2026.07.21-2 — M7 B2/B3: checkout Terms + immediate-start consent, portal-only plan changes for paid subscriptions — deployed 2026-07-21 (code-only, no migration)

- **Release tag:** `v2026.07.21-2` (deployed commit = main HEAD `00b559f`, squash-merge of PR #233).
- **Live Worker Version ID:** `a057a603-cb5c-4679-a4be-823cdb819c04` (verified at `GET /health`, 6/6 cache-busted probes after propagation).
- **Rollback Worker Version ID:** `67012af4-3880-4066-8777-e84a6333d0df` — this was the actual pre-deploy live version: `b6a39ff` code (v2026.07.20-11) **plus** the founder's 20 Jul sandbox Stripe secret configuration ("Secret Change" version, 20 Jul 22:15 UTC). **Do not roll back to `239bcd77`** — same code but pre-Stripe-config secrets; a rollback there would silently restore stale secrets.
- **Migration:** none. Latest applied remains `098-related-changes.sql`. **No Stripe prices, products, secrets or webhook endpoints changed in this release.** Frontend already live via Pages (deployed with the #233 merge; runtime delta `b6a39ff → 00b559f` re-verified pre-deploy as exactly the three approved Worker files: `engines/stripe.js`, `routes/billing.js`, `routes/global-billing.js`).
- **B2 — checkout consent:** every Stripe Checkout Session now sets `consent_collection[terms_of_service]=required` (mandatory Terms checkbox) with `custom_text` stating the approved policy verbatim: the digital service starts immediately → the 14-day consumer cancellation (cooling-off) right is given up; prepaid fees are non-refundable except where required by law; the free 14-day trial exists to try first. Stripe records the acceptance on the session; the `checkout.session.completed` webhook writes `terms_consent` into the audit event and subscription event trail.
- **⚠️ Amended Stripe ToS precondition (founder-verified 21 Jul):** the sandbox Terms-of-Service URL could NOT be set — per Stripe docs, sandbox public details are not copied from live and Stripe injects placeholder domains ("to enable payments in a sandbox"); this sandbox's dashboard renders business-details only as an "incomplete" state. **Deployed anyway by founder decision.** The B2 checkout acceptance test (founder-run, post-deploy) empirically verifies `consent_collection` against the placeholder public details; if session creation fails with a missing-ToS-URL error, that is the fail-closed path working (surfaces as `stripe_api_error`, never a silent skip) and must be reported, not worked around. The real Terms URL (`https://cybermeters.com/terms`) is a LIVE-cutover checklist item on the live account.
- **B3 — portal-only plan changes (stacking question answered definitively):** a subscription-mode Checkout Session always creates a NEW Stripe subscription and nothing in the checkout/webhook chain cancels a prior one — so a fresh checkout by an existing paid subscriber WOULD have stacked a second live subscription and billed both. Structural fix: the checkout route resolves the canonical subscription and, for an active paid Stripe-bound sub (active, or past_due inside the payment-grace window), returns a **billing-portal session** in the same `{url}` response shape (self-heals older cached frontends) with a `billing_checkout_routed_to_portal` audit event. Trial/free/manual rows keep the fresh-checkout path. Frontend paid subscribers see "Change plan in billing portal".
- **Validation (all on `00b559f` pre-deploy):** `validate-checkout-consent-portal.js` 28/28 (incl. 4 load-bearing mutations); full 159-validator sweep green; frontend 48 files / 416 tests green; Worker syntax, wrangler dry-run, `git diff --check` clean.
- **Production proof:** `GET /health` → `a057a603`, 6/6 cache-busted. The controlled B2 checkout-consent test and the B3 single-subscription plan-change test are deliberately **not** run in this deploy task — they are founder acceptance actions.
- **Founder acceptance next:** (1) sandbox checkout → Terms checkbox + waiver text render, `terms_consent: "accepted"` in the audit trail — or the documented fail-closed missing-ToS-URL error; (2) account with ONE active sub → in-app "Change plan in billing portal" → Stripe switches the existing subscription with proration and no second subscription appears.
...
- **Live Worker Version ID:** `239bcd77-366a-4263-b72d-c12bcac35742` (verified at `GET /health`, 6/6 cache-busted probes after propagation).
- **Rollback Worker Version ID:** `0b2354e7-4ca8-4f9d-b122-02d89b207d7e` (v2026.07.20-9, M7 runtime).
- **Migration:** none. Latest applied remains `098-related-changes.sql`. No frontend change.
- **Release-shape note (recorded deliberately):** the founder's original brief specified an ISOLATED v8+P1 Worker artifact (cherry-pick onto `v2026.07.20-8`) on the premise that `761f2f50` was live and M7 undeployed. Pre-deploy verification found the **M7 runtime already live** (`0b2354e7`, deployed 15:09 UTC, recorded under v2026.07.20-9 above). The discrepancy was surfaced and the founder ruled: **deploy main** (M7 stays live as it already was; this release adds ONLY the P1 fix on top). The isolated artifact was never built or deployed.
- **Symptom:** since 19 Jul every completed scan platform-wide had `scan_quality='partial'` with `modules_skipped=['asset_exposure']` and warning "Module incomplete: asset_exposure" (last complete scan 18 Jul 14:06) — blocking the M6 A6 acceptance baseline and pushing every weekly digest toward the honest no-completed-assessment state.
- **Root cause (reproduced read-only from production R2 + live public probes, no `wrangler tail` needed):** inside a Worker, `fetch()` to a hostname with no functioning origin does not throw — the Cloudflare edge synthesises a **52x/530 response** (evidence: `reports.`/`send.cybermeters.com` — mail-only names that do not publicly resolve — recorded **530**/1016; `api.cybermeters.com` — publicly a plain 404 — recorded **522**; `email.blackbullbarbers.co.uk` recorded **526**). #185's App-Probe Reliability blanket `status>=500 → probe_status:'server_error'` classified these edge answers as "origin answered 5xx, not content-assessable", and `runExposureModule` escalates any such host to module `incomplete` → scan `partial`. Every real domain carries a DNS-only/mail-only subdomain in its CT/brute-force list, so every scan went permanently partial.
- **Fix (`probeAsset`, shared by legacy + reserved paths):** a 5xx in **520–530 carrying the `Server: cloudflare` edge signature** classifies as `probe_status:'origin_unreachable'` / `reason:'cloudflare_edge_error'` — an authoritative negative (same evidence class as connection refused; `reachable` stays `false`, real status preserved on the asset record) excluded from the NOT_ASSESSED set. A genuine origin 5xx (500–511, or any 5xx without the edge signature, including a proxied origin's own 502) keeps the #185 server_error → not-assessed → partial contract unchanged. Scan-quality semantics are NOT relaxed — timeouts, budget starvation and real server errors still degrade honestly.
- **Validation:** `validate-app-probe-reliability.js` extended to **71/71** — +12 assertions (edge classification incl. proxied-origin-502 and unsigned-530 strictness; B8 production-shape repro; B9 mixed edge+origin-5xx still partial; D6 end-to-end `scan_quality: complete`) and +3 load-bearing mutations (M15 edge range widened to all 5xx, M16 signature check dropped, M17 edge branch removed = the P1 reintroduced — each fails the suite). Full **158-validator** sweep green on the deployed commit; regression fixtures 220/220; Worker syntax + dry-run + `git diff --check` clean.
- **Production proof:** `GET /health` → `239bcd77` (6/6 cache-busted). The controlled cybermeters.com acceptance scan + R2 `scan_quality` verification are deliberately deferred to the founder acceptance session per the release brief.
- **M7 status unchanged:** M7 remains live exactly as deployed under v2026.07.20-9/-10 — still **PENDING FOUNDER STRIPE CONFIGURATION** per `docs/M7-STRIPE-CUTOVER-ACTIONSHEET.md`; no Stripe objects/secrets/config were touched by this release.

## v2026.07.20-9 / -10 — M7 Pricing & Billing Lockstep (PR-1 runtime + PR-2 frontend) — deployed 2026-07-20 (code-only, no migration)

- **Release tags:** `v2026.07.20-9` (runtime, squash-merge `6d3beec`, PR #225) and `v2026.07.20-10` (frontend-only, squash-merge `b75cd51`, PR #226).
- **Live Worker Version ID:** `0b2354e7-4ca8-4f9d-b122-02d89b207d7e` (verified at `GET /health`, cache-busted).
- **Rollback Worker Version ID:** `761f2f50-00bf-4266-b4ad-9bc34941b780` (v2026.07.20-8).
- **Migration:** none. Latest applied remains `098-related-changes.sql`. Duplicate production subscription rows stay append-only untouched — the new resolver makes them deterministic.
- **Status:** **M7 ENGINEERING COMPLETE / PENDING FOUNDER STRIPE CONFIGURATION.** Nothing is sellable until the founder creates the canonical Stripe prices and sets the secrets — `docs/M7-STRIPE-CUTOVER-ACTIONSHEET.md` is the exact action sheet. The checkout guard refuses any Stripe price that does not charge the policy amount, so the stale test prices cannot sell in the meantime.
- **What shipped (PR-1, runtime):**
  - **Canonical pricing registry** (`engines/pricing-registry.js`) — the one machine-readable projection of `docs/PRICING-POLICY.md` (LOCKED 2026-07-19): Trial £0/14d/1 domain · Starter £9.99/1 · Professional £19.99/3 · Business £49.99/10 included (+£3/mo per-domain model to 25, declared) · MSP £99.99 base + £3/domain min 10 (floor £129.99) · annual = exact ×10. `BILLING_PLAN_METADATA` and the `PLAN_LIMITS` domain ladder derive from it; the legacy £29/£149/£399 metadata is gone; additive `pricing_model` API fields expose the per-domain commercial model.
  - **Trial fail-closed** — a `trialing` row entitles only inside its trial window (`trial_end`, fallback `current_period_end`; neither → free). Fixes the defect where locally-provisioned 14-day trials granted Professional **forever** at runtime (trial rows never set `current_period_end`, the only field the old resolver checked).
  - **Canonical subscription resolver** (`resolveCanonicalSubscriptionRow`) — lineage supersession + explicit precedence (paid > trial > grace), replacing the ambiguous newest-row `ORDER BY … LIMIT 1` (the F0 duplicate-rows precondition); shared by runtime entitlements AND the workspace-subscription API so UI and feature gates cannot disagree.
  - **Fail-closed Stripe mapping** — webhook price→plan uses the merged map (individual env vars + JSON) in both directions; a present-but-unknown price grants NO plan (plan unchanged + `stripe_unknown_price_fail_closed` audit event), never the metadata fallback; multi-item subscriptions resolve the base item by mapped price.
  - **Checkout price-policy guard** (`verifyStripePriceMatchesPolicy`) — both checkout routes read the resolved price from Stripe and refuse the session unless it charges exactly the policy amount (GBP, interval, pence, active).
  - **Trial domain cap = 1** at all three domain-add enforcement sites; Business domain-add beyond the 10 included refused honestly (`additional_domain_billing_unavailable`) until per-domain overage billing is wired; `free` = post-trial read-only (0 new scans/reports/schedules) per policy §4; honest cancellation/payment-failure email copy.
- **What shipped (PR-2, frontend):** PricingPage renders ONLY live API pricing (stale hard-coded fallback removed; API failure → honest "live pricing unavailable" + retry, never a stale figure); trial card, Business per-domain expansion line and MSP base/floor pricing from `pricing_model`; exact ×10 annual figures. SubscriptionPage drops its local price table and the hard-coded "£29/mo"; PlanUsageCard upgrade-wall claims align to the locked ladder (3/10, previously 5/20). New PricingParity test guards against any legacy price or local price table reappearing.
- **Validation:** new CI-blocking `validate-pricing-policy-parity.js` (68 checks); `validate-billing-lifecycle` 23/23 (+7); `validate-stripe-events` 20/20 (+6); 8/8 mutation probes caught (swap price mappings, metadata-fallback restore, never-expiring trial, cancel-at-period-end-as-immediate, MSP-to-business grant, stale price, amount-check removal, trial-cap removal); full **158-validator** sweep green; FE 47 files / 406 tests + coverage + build green.
- **Production proof (side-effect-safe):** `GET /health` → `0b2354e7`; `GET /api/plans` serves the canonical ladder (9.99/19.99/49.99, domains 1/3/10, MSP floor 129.99) — confirmed live post-deploy; `POST /api/billing/checkout` unauthenticated → 401 (route live + auth-gated). Authenticated checkout cannot be proven until founder Stripe configuration (guard by design).
- **Founder-visible entitlement transitions (all accounts are founder-controlled; no external customers exist):** the two owners on **expired** trials (`cbtacar@gmail.com`, `blackbullbarbers@gmail.com`) now resolve honestly to free (read-only) instead of defect-granted Professional; `ttrnn47@gmail.com` resolves to its real Stripe-bound **starter** row once its trial window (ended 14:11 UTC today) lapses — alerts (starter+) continue, Professional-only surfaces (CE analytics, executive dashboard, vendor risk, audit logs) gate off; `hello@blackbullbarbers.co.uk` (trial to 28 Jul) and `hello@cybermeters.com` (trial to 2 Aug) keep Professional via their live trials. Remedy for any account needed in acceptance: test-mode checkout per the action sheet.
- **Outstanding (founder):** Stripe price creation + secrets per `docs/M7-STRIPE-CUTOVER-ACTIONSHEET.md`; per-domain overage billing (Business 11–25, MSP metered) is a follow-up engineering episode gated on those prices — until then it is honestly refused, never silently given away.
...
- **Live Worker Version ID:** `f06e95a2-be19-4cf0-9794-30162cb77368` (verified live at `GET /health`; deployed 2026-07-20 from main `20730c9`).
- **Rollback Worker Version ID:** `1fdbcf1e-331b-4447-962d-aaabcca35963` (v2026.07.20-1, PR-A).
- **Migration:** none. Latest applied production migration remains `098-related-changes.sql`.
- **Sprint context:** stage 2 of the three-PR Brand Protection blocker-remediation sprint. PR-A added the lookalike *bases* (TLD swaps + credential keywords); PR-B discovers the *nested hosts* on those bases, closing the founder's exact example `acme.com` → `office365password.acme.co` — undiscoverable by generation alone (arbitrary nesting cannot be enumerated). **Brand Protection remains PUBLIC-BETA BLOCKED until PR-C also passes its production acceptance gate.**
- **What shipped (engine + daily cron + one frontend badge; no migration):**
  - **`engines/brand-passive-discovery.js`** — a bounded daily cron task (`brand_passive_discovery`, 03:00 UTC) queries crt.sh once per workspace on the brand **token** (a CT identity search that surfaces the token anywhere in a CN/SAN, including nested hosts), then applies a **strict registrable-base membership filter**: a discovered FQDN is kept only when its registrable base (eTLD+1, via the reused `getRegisteredDomain`) is one of the lookalike bases the PR-A generator already produces. `office365password.acme.co` is kept (base `acme.co` is a generated lookalike); an unrelated legitimate `acme-widgets.com` and the customer's own `www.acme.com` are both dropped. This membership rule is the load-bearing false-positive control.
  - **One enrichment path, not two:** discovered hosts land `dns_resolves NULL`, so the existing hourly `brand_dns_enrichment` sweep validates them on its next tick.
  - **CT-observed evidence honesty:** new `variant_type: "nested_host"` and an `evidence_json` `ct_observed` signal (both additive; the column is free-text). Customer copy is deliberately factual — **"Seen in certificate log"** / **"Subdomain of a lookalike"**. A CT observation means a certificate *naming* the host was logged; it does not claim the host is live, serving, or malicious.
  - **Risk-band honesty:** `ct_observed` lifts a candidate out of the unregistered-watchlist cap (real evidence the host was provisioned) but, without a confirmed-live DNS/MX signal, is **capped at `high`, never `critical`**. The `brandCandidateToApi` serializer reads the durable `ct_observed` signal into its recompute, so the API band can never silently disagree with the persisted band.
  - **No silent truncation:** bounded to 3 workspaces/day and 50 hosts/workspace/run; the sweep logs the number of eligible workspaces deferred by the cap. Full multi-workspace coverage is a post-beta scaling item.
- **Honest limits (asserted, deferred to PR-C):** no HTTP/TLS live-surface signal is set here (`https_active` / `looks_like_login` from a live probe is **PR-C**); discovery is cron-only (not added to the customer-facing `/brand-monitoring/refresh` request, to keep its subrequest budget bounded); the filter finds nested hosts on bases PR-A generates and does not invent new lookalike bases from CT (by design — that reintroduces false positives).
- **Safety:** additive and reversible (cron task, `variant_type`, `ct_observed` signal all removable without data loss). CT query hits a fixed host (crt.sh) with a URL-encoded query param — no user-controlled host, no SSRF surface. Transient CT failures never fabricate state. Tenant isolation asserted end-to-end.
- **Tests:** `scripts/validate-brand-passive-discovery.js` (52 checks, CI-blocking) — the founder regression matrix, membership/false-positive control, CT-observed risk honesty, serializer band agreement, idempotency, tenant isolation, transient-failure safety; **mutation-proven** (7 deliberate reversions each fail the suite). `validate-pipeline.js` cron task-set expectation aligned for the new hour-3 task.
- **Acceptance:** proven by CI validators, frontend tests (54/54) and read-only production smoke (`/health` serving `f06e95a2`, `/ready` 200, brand route auth-gated 401). Founder acceptance of a controlled scan showing a real CT-discovered nested host persisted, DNS-swept and banded honestly remains a sprint release-gate activity.

## v2026.07.20-1 — Brand Protection PR-A: TLD-substitution + credential-keyword lookalike coverage — deployed 2026-07-20 (code-only, no migration)

- **Release tag:** `v2026.07.20-1` (points at squash-merge commit `c76c211906d5d03d10d4418a3bae50d802edeb72`, PR #204).
- **Live Worker Version ID:** `1fdbcf1e-331b-4447-962d-aaabcca35963` (verified live at `GET /health`; deployed 2026-07-20 from main `c76c211`).
- **Rollback Worker Version ID:** `5239fbb9-6425-4b73-8f54-13c61666b0cc` (A5 executive PDF release).
- **Migration:** none. Latest applied production migration remains `098-related-changes.sql`.
- **Sprint context:** stage 1 of the founder-approved three-PR Brand Protection blocker-remediation sprint (PR-A → PR-B → PR-C). The 2026-07-20 read-only audit found the typosquat generator TLD-locked — every candidate reused the customer's own TLD, so `acme.com` could never produce `acme.co` — and its keyword sets missing the credential-phishing vocabulary. The most common real-world impersonation pattern (exact brand token on a confusable TLD) was structurally undetectable, and the generator had zero unit tests. **Brand Protection remains PUBLIC-BETA BLOCKED until PR-A, PR-B and PR-C have all passed their production acceptance gates.**
- **What shipped (generator + tests only; pure computation, no network, no frontend change):**
  - **TLD substitution:** the exact brand label is emitted over a curated confusable-TLD list (`com, co, net, org, io, app, uk, co.uk, info, biz`) as `variant_type: tld_variation` — already canonical in `normalizeBrandVariantType` and already weighted (18) in `scoreBrandCandidateRisk`; the risk model was scaffolded but unwired. The customer's own registered domain is never emitted, including via identity-preserving transpositions of double letters.
  - **Credential keyword expansion:** `password, office365, o365, m365, microsoft, otp, mfa` join the HIGH-risk set; `reset, unlock` join the MED set. These also feed the existing `looks_like_login` inference at persistence time.
  - **Speculative-risk honesty:** TLD-swap candidates are born `medium` (generator score 4, deliberately below the `high` threshold), because `computeScore` converts high/critical generated candidates into scored findings (−3/−5, capped −15) — an **unregistered** permutation must never move a customer's Brand score. Customer-facing risk stays owned by `scoreBrandCandidateRisk`, whose registration-reality gate caps unregistered candidates at `low` and lifts DNS/MX-confirmed ones to at least `high`. Verified: zero high/critical candidates at generation.
  - **Family-quota selection** inside the unchanged 40-candidate cap (tld ≤ 10, keyword ≤ 14, character variants ≤ 16, score-ordered backfill). Without it the enlarged keyword set ejects the homoglyph/typo family to 1-of-40 for an 11-letter brand.
  - **`.co` deliberately NOT added to `BRAND_SUSPICIOUS_TLDS`** (conscious deviation from the audit note): that set is TLD-intrinsic; swap-ness is candidate-relative and already scored. Adding it would mis-penalise legitimate `.co` customers.
  - **Tests:** `scripts/validate-brand-typosquat.js` (51 checks, CI-blocking — the generator's first tests) covering the founder regression matrix, speculative-risk honesty, quotas, determinism, multi-label TLDs and the own-domain guard; **mutation-proven** (5 deliberate reversions each fail the suite).
- **Honest limits (asserted in the suite, closed by later stages):** nested credential hosts (`office365password.acme.co`) remain undiscoverable until **PR-B** (passive CT/SAN discovery — the only reliable route); multi-keyword forms are not generated; no HTTP/TLS live-surface enrichment until **PR-C**.
- **Also shipped en route — PR #203 (`8c1abd2`, test-only):** `validate-cert-trust-l2.js` fixture date-rot repaired. Its "current certificate" fixture expired `2026-07-20T00:00:00Z`, so the `parallel_certificate` anomaly stopped firing and **all CI on main went red on 2026-07-20 with no code change**. Currently-valid fixtures are now expressed relative to run time.
- **Operational note (Cloudflare Pages, infra-side):** three consecutive Pages builds failed around 00:15–01:00 UTC on 2026-07-20 (`e608ec8e` production `8c1abd2`, `04aaf326` preview `32eb331`, `225b545d` production `c76c211`) with no frontend diff in any of them and a green local build; the GitHub Pages check also stalled (stale `in_progress` for a failed build). Production Pages continues serving the last-good, content-identical build (`134fca15`, `aa418fc`) — no customer impact. **Follow-up: inspect Cloudflare dashboard build logs / retry when the build service recovers.**
- **Acceptance:** proven by CI validators and read-only production smoke (`/health` serving `1fdbcf1e`, `/ready` 200, brand and case routes auth-gated 401). A `401` proves the route is live, not the authenticated workflow. Founder production acceptance of the Brand Protection sprint (controlled scan showing `tld_variation` candidates persisted and DNS-swept) remains a sprint release-gate activity.
