...
| 2 | Independent retest | 🟡 | Every fix independently re-verified vs HEAD + live smoke by a second reviewer. Full internal pentest run against the 459-control checklist by the Claude/Turhan/Codex/ChatGPT team — see `docs/PENTEST-CHECKLIST-INTERNAL.md` (coverage map: ~14 areas CI-proven, focused manual queue for SSO/XSS/SSRF/business-logic). **No third-party firm yet (cost)** — an independent external test stays scheduled before commercial scale (P2-1); the Trust Center does not claim otherwise. |
