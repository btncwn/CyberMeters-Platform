...
|---|---|---|---|
| **DISCOVERED 2026-07-08: Workers Builds auto-deploys `cybermeters-platform` on every push to main** (version timestamps correlate 1:1 with pushes since 09:37 UTC; live `671519e4` = the e126dfd docs push, not a wrangler deploy). This silently bypasses the CLAUDE.md deploy-approval gates and makes push ≙ deploy for the api worker | certain (observed) | high (unreviewed deploys; breaks Stage C choreography) | **founder decision required before Stage C**: either disconnect Workers Builds for the worker (recommended — restores explicit `wrangler deploy` gating; Pages auto-deploy for the frontend stays) or move to a `production` deploy branch. Until decided: only behaviour-identical worker changes may be pushed to main |
