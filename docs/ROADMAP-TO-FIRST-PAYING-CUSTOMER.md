...
Current canonical episode: **M6 — External Behaviour Intelligence Engine** (see the Pre-Public-Beta Gate Sequence below). **M5 Completion Across All Eight Domains is engineering-complete: M5.a–M5.g all CLOSED.**
...
- **M6** — External Behaviour Intelligence Engine — gated by **M6.0** (see below). **M6.0-A** (competitive differentiation hypothesis) **PASSED**; build viability is **not** yet proven. **M6.0-B** (sensor sufficiency + real-data proof) is the **CURRENT ACTIVE PHASE** — a read-only, no-code empirical study on the two founder domains + a test-domain corpus. M6 is deliberately NOT "change over time" (the existing posture-diff timeline already does that); its only defensible new value is cross-domain correlation + bounded deterministic significance. Founder decision (18 July 2026): M6 precedes pricing (M7) so pricing reflects the engine's proven value.
- **M7** — Pricing + Billing Alignment — follows M6.
...
| 7 | **M6.0 Viability Gate → M6 (or RESCOPE)** | **CURRENT ACTIVE PHASE** — M6.0-A differentiation hypothesis PASSED; M6.0-B sensor-sufficiency + real-data proof OPEN (read-only, no code). Founder GO → narrow M6 build; RESCOPE → "Posture Timeline Trust & Actionability"; DEFER |
...
- **7 — M6.0 Viability Gate, then M6 or RESCOPE.** Honest re-diagnosis: the app ALREADY does change-over-time (`posture-events.js` is a posture-diff engine feeding the Exposure Timeline + weekly digest). So **M6 is NOT "change over time"** — its only defensible new value is **cross-domain correlation + bounded DETERMINISTIC significance** the existing diff timeline cannot produce. Significance is composed deterministically (`change type + security direction + persistence + recurrence + inventory mismatch + corroborating signals`; historical rarity is one enriching factor, not the base) — **not** statistical anomaly; customer-facing wording avoids "anomalous/suspicious/attacker/statistically unusual" in favour of "material / correlated / requires verification / higher-confidence progression". Correlation is kept small (**≤3 signal families** per use case in v1). Deterministic decides, AI explains; change ≠ compromise; honest-by-default (insufficient baseline → "still learning", never a fabricated confidence number); emits through the canonical alert/case/remediation systems (no parallel platform).
  - **M6.0-A — Competitive differentiation hypothesis: PASSED** (structurally verified: provider audit logs — Cloudflare, GoDaddy — are control-plane-only / own-account / external-blind; the external, cross-provider and attacker-side blind spots are real). Build viability is **not** yet proven.
  - **M6.0-B — Sensor sufficiency + real-data empirical proof: OPEN** (read-only, no code, no migration, timeboxed). Candidate use cases, ranked: (1) Brand weaponisation progression, (2) Email posture transition, (3) New public surface correlation. Every use case must pass **three counterfactuals** — provider, existing-timeline, and analyst-at-scale (or, for SMEs, continuity) — and show a single actionable output. **Founder domains prove honesty + noise control; the test-domain corpus proves detection.**
  - **Outcome: founder GO / RESCOPE / DEFER.** RESCOPE fallback = **"Posture Timeline Trust & Actionability"** (producer-version guard, `not_comparable`/unavailable/insufficient semantics, actionability, ownership, dedupe) — valuable, but it must NOT carry a "behaviour intelligence" claim. No schema, migration or implementation before GO.
  - **Scanner boundary (canonical):** CyberMeters will not embed or depend on open-source vulnerability scanner engines or community scanning-template ecosystems in its production scanning pipeline. External checks must be proprietary CyberMeters probes or separately founder-approved commercial integrations. Public vulnerability data such as KEV, NVD and CVE metadata may be used only for enrichment and prioritisation; it must not independently create or verify a finding. No new scanner dependency may be introduced implicitly during M6. M6 consumes selected existing, inventoried and approved sensor outputs; developing a new vulnerability detection framework is outside M6 and requires a separate founder-gated programme.
  - Founder decision (18 Jul 2026): the engine precedes pricing (M7) so pricing reflects its proven value.
...
The canonical order is the Pre-Public-Beta Gate Sequence above. **M5 (a–g) is engineering-complete; the active phase is the M6.0 viability gate.** The immediate sequence:

1. Run **M6.0-B** — sensor-sufficiency + real-data empirical proof (read-only, no code, no migration): per candidate use case (Brand weaponisation progression #1, Email posture transition #2, New public surface #3) prove producer + comparable history + eligibility + deterministic change/correlation, passing the provider / existing-timeline / analyst-at-scale (or SME-continuity) counterfactuals, with founder-domain honesty/noise proof and controlled-fixture detection proof. Then founder **GO** (narrow M6 build) / **RESCOPE** (Posture Timeline Trust & Actionability) / **DEFER**. No schema, migration or implementation before GO.
2. Complete **M7 Pricing + Billing** lockstep (Stripe + entitlements + pricing cards, one release, founder-approved), priced with the engine's proven value; architecture with a Behaviour Intelligence tier.
