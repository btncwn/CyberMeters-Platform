...
export const REMEDIATION_REGISTRY = Object.freeze([
  ...
  entry({
    ...
    technical_explanation: "A complete CyberMeters DMARC lookup found no applicable policy, or found that a previously observed exact record was lost without equal or stronger inherited policy. This observation does not prove how every receiver handled a message.",
    business_impact: "Without an equal or stronger applicable published policy, receivers have no equivalent DMARC-requested quarantine or reject disposition to consider for this domain.",
    recommended_action: "Ask your DNS or email administrator to publish one valid DMARC record. Begin with a no-action policy (p=none) and aggregate reporting while legitimate senders are reviewed, then strengthen the requested policy when ready. CyberMeters will not apply the DNS change.",
    effort: "medium",
    owner_type: "customer_it",
    verification_method: "dns_recheck",
    verification_evidence_requirements: "A later complete CyberMeters DNS re-observation finds an applicable valid DMARC policy.",
  ...
  entry({
    ...
    technical_explanation: "The complete current observation shows a no-action effective requested DMARC policy. Historical findings bound to this remediation may instead preserve a legacy pct value; DMARCbis does not apply that legacy value to the current effective policy. A published request does not prove how every receiver handles messages.",
    business_impact: "A no-action policy requests reporting without requesting quarantine or rejection of messages that fail DMARC.",
    recommended_action: "Confirm legitimate sending sources pass alignment, then ask your DNS or email administrator to move the requested policy towards quarantine or reject when ready. CyberMeters will not apply the DNS change.",
    effort: "medium",
    owner_type: "customer_it",
    verification_method: "dns_recheck",
    verification_evidence_requirements: "A later complete CyberMeters DNS re-observation derives an effective requested quarantine or reject policy.",
    supporting_evidence_types: ["dns_txt_lookup"],
    managed_workflow_compatible: true,
  }),
  entry({
  ...
  entry({
    ...
    technical_explanation: "A complete CyberMeters lookup observed multiple DMARC policy records at the same DNS name, so no single authoritative policy can be derived.",
    business_impact: "The ambiguous record set prevents CyberMeters from deriving one applicable requested policy and may prevent receivers from applying DMARC processing.",
    recommended_action: "Ask your DNS or email administrator to merge the required tags into one valid DMARC record at the observed DNS name and remove the duplicates. CyberMeters will not apply the DNS change.",
    effort: "low",
    owner_type: "customer_it",
    verification_method: "dns_recheck",
    verification_evidence_requirements: "A later complete CyberMeters DNS re-observation finds that the multiple-record condition is absent.",
    supporting_evidence_types: ["dns_txt_lookup"],
    managed_workflow_compatible: true,
  }),
  entry({
    ...
    technical_explanation: "A complete CyberMeters lookup observed a DMARC policy record that cannot be used as a valid policy.",
    business_impact: "A malformed policy can prevent a dependable DMARC disposition from being derived even though a record appears to be published.",
    recommended_action: "Ask your DNS or email administrator to correct the record using the observed parser evidence, preserving required reporting destinations where valid. CyberMeters will not apply the DNS change.",
    effort: "low",
    owner_type: "customer_it",
    verification_method: "dns_recheck",
    verification_evidence_requirements: "A later complete CyberMeters DNS re-observation finds that the malformed-record condition is absent.",
    supporting_evidence_types: ["dns_txt_lookup"],
    managed_workflow_compatible: true,
  }),
  entry({
    remediation_id: "email.dmarc.external_rua_authorization",
    domain_key: "email_protection",
    finding_types: ["dmarc_external_rua_unauthorised"],
    customer_title: "Authorise the external DMARC report destination",
    technical_explanation: "A complete CyberMeters external-authorisation lookup found no valid RFC 9990 authorisation for an aggregate-report destination outside the policy domain.",
    business_impact: "Receivers may omit aggregate reports to an external destination that has not published the required authorisation.",
    recommended_action: "Ask the destination operator and your DNS administrator to publish or correct the required external-report authorisation record, or remove the destination if it is not intended. CyberMeters will not apply either DNS change.",
    effort: "low",
    owner_type: "customer_it",
    verification_method: "dns_recheck",
    verification_evidence_requirements: "A later complete CyberMeters DNS re-observation finds the unauthorised external-destination condition absent.",
    supporting_evidence_types: ["dns_txt_lookup"],
    managed_workflow_compatible: true,
    limitations: [
      "Authorisation permits report delivery; it does not prove that a report was sent, received or trusted for ingestion.",
      "A DMARC policy request does not prove receiver enforcement.",
    ],
  }),
  entry({
  ...
  entry({
    ...
    finding_types: ["ssl_certificate_expired", "certificate_expired"],
  ...
  entry({
    ...
    version: 2,
    domain_key: "certificates_trust",
    finding_types: ["ssl_certificate_expiring_soon", "certificate_expiring_soon", "certificate_expiring_critical"],
    ...
    owner_type: "customer_it",
    verification_method: "certificate_recheck",
    verification_evidence_requirements: "A replacement certificate with a later expiry for the exact host is observed through Certificate Transparency or an HTTPS probe.",
    supporting_evidence_types: ["certificate_transparency_observation", "https_probe"],
  }),
  entry({
  ...
  entry({
    ...
    customer_title: "Add or strengthen the Content Security Policy",
    ...
    owner_type: "customer_it",
    verification_method: "https_recheck",
    verification_evidence_requirements: "A parseable Content-Security-Policy is re-observed and the admitted script-src/default-src dangerous-token condition is no longer present.",
    supporting_evidence_types: ["http_header_probe"],
  }),
  entry({
  ...
  entry({
    ...
    technical_explanation: "Cookies were set without the Secure, HttpOnly and/or SameSite attribute named by the finding.",
    business_impact: "Cookies without these flags can be sent over HTTP or read by scripts, increasing session-hijacking risk.",
    recommended_action: "Set Secure and HttpOnly on session cookies, and set SameSite=Lax or SameSite=Strict according to the application's cross-site requirements.",
    effort: "low",
    owner_type: "customer_it",
    verification_method: "https_recheck",
    verification_evidence_requirements: "At least one cookie is re-observed and the Secure, HttpOnly or SameSite attribute named by the finding is no longer missing; observing zero cookies is inconclusive.",
    supporting_evidence_types: ["http_header_probe"],
  }),
  entry({
  ...
  entry({
    ...
    customer_title: "Historical Microsoft 365 provider observation",
    technical_explanation: "This historical finding ID represented provider-identification evidence. It did not measure a reachable endpoint, tenant configuration, MFA or Conditional Access.",
    business_impact: "Provider identification alone does not establish a security weakness or reachable sign-in surface.",
    recommended_action: "Review the current provider relationship and possible identity-facing hostname evidence; no endpoint remediation is inferred from this historical ID.",
    effort: "medium",
    owner_type: "customer_it",
    verification_method: "unsupported",
    verification_evidence_requirements: "Unsupported: CyberMeters does not currently measure the endpoint or tenant configuration represented by this historical ID.",
  ...
  entry({
    ...
    status: "deprecated",
    deprecated_at: "2026-08-11",
    customer_title: "Historical legacy-authentication observation",
    technical_explanation: "This historical finding ID has no implemented legacy-authentication producer. Provider or hostname evidence does not establish that legacy authentication is enabled or reachable.",
    business_impact: "No current customer risk conclusion can be drawn from this unsupported historical ID.",
    recommended_action: "No CyberMeters remediation is inferred; confirm legacy-authentication policy directly in the identity provider if your own governance requires it.",
    effort: "medium",
    owner_type: "customer_it",
    verification_method: "unsupported",
    verification_evidence_requirements: "Unsupported: no CyberMeters legacy-authentication measurement exists.",
    supporting_evidence_types: ["identity_discovery"],
  ...
  entry({
    ...
    status: "deprecated",
    deprecated_at: "2026-08-11",
    customer_title: "Historical Autodiscover hostname observation",
    technical_explanation: "This historical finding ID has no implemented Autodiscover reachability producer. A possible hostname is not a measured endpoint.",
    business_impact: "No current reachability or weakness conclusion can be drawn from this unsupported historical ID.",
    recommended_action: "Review the possible hostname with its owner; no endpoint restriction is inferred from this historical ID.",
    effort: "medium",
    owner_type: "customer_it",
    verification_method: "unsupported",
    verification_evidence_requirements: "Unsupported: no CyberMeters Autodiscover endpoint measurement exists.",
    supporting_evidence_types: ["identity_discovery"],
    limitations: ["Identity Exposure covers exposed sign-in surfaces only, not credential-breach or dark-web data."],
  }),
  ...
  entry({
    ...
    customer_title: "Review changed identity-provider evidence",
    technical_explanation: "The observed provider relationship or hostname classification changed. The previous evidence is preserved; this does not establish endpoint reachability or a tenant-configuration change.",
    business_impact: "A changed provider relationship may reflect an intended migration or a discovery change. It is review evidence, not proof of a weakness or hijacked sign-in route.",
    recommended_action: "Confirm whether the provider relationship or possible hostname is expected, record the classification, and assign an owner if follow-up is needed.",
    effort: "medium",
    owner_type: "customer_it",
    verification_method: "manual_attestation",
    verification_evidence_requirements: "The review decision and owner are recorded. CyberMeters does not verify endpoint reachability or tenant configuration from this observation.",
    supporting_evidence_types: ["identity_discovery"],
    limitations: ["Provider-identification evidence does not establish a reachable sign-in surface, leaked credentials, breached passwords or dark-web exposure."],
  }),
  entry({
